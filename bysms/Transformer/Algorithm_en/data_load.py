...
def load_train_data():
    en_sents = [regex.sub("<[^>]+>", "", line) for line in codecs.open(hp.source_train, 'r', 'utf-8').read().split("\n") if line and line[0] != "<"]
    cn_sents = [regex.sub("<[^>]+>", "", line) for line in codecs.open(hp.target_train, 'r', 'utf-8').read().split("\n") if line and line[0] != "<"]
...
def load_test_data():
    ...
    cn_sents = [_refine(line) for line in codecs.open(hp.target_test, 'r', 'utf-8').read().split("\n") if line ]
...
def load_test_data1():
    ...
    en_sents = [_refine(line) for line in codecs.open(hp.source_test1, 'r', 'utf-8').read().split("\n") if line ]
    cn_sents = [_refine(line) for line in codecs.open(hp.target_test1, 'r', 'utf-8').read().split("\n") if line ]
